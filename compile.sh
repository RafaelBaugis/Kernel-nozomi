...
cd $WORKDIR/vendor/broadcom/wlan/dhd/linux && make clean
cd $LINUXDIR && make clean && make mrproper && make distclean
...
cd $WORKDIR/vendor/semc/build/$FS && find . | cpio -o -H newc > ../ramdisk/boot/ramdisk.cpio
